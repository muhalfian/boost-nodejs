...
exports.createRoutes = function(app_ref) {
  ...
  app.get('/admin/:action', adminRoute);
  app.post('/emotion/:action', emotionRoute);
  app.get('/chart', chartRoute);
  app.get('/', musicRoute);
...
function chartRoute(req, res) {
  ...
  Emotion.find({}).exec({}, function(err, emotions) {
      ...
      // res.json(emotions);
      res.render('emotions', {emotions:emotions});
  });
    res.render('charts', {});
};

var emotionRoute  = require('./emotion.js');
